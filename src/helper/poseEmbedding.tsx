...
export class PoseEmbedding {
  ...
  normalize_pose_landmarks(landmarks: any[] ) {
    console.log("origin_landmarks",landmarks)
    ...
    for (let i = 0; i < landmarksArr.length; i++) {  
        landmarksArr[i].x -= pose_center[0]; // 减去x坐标  
        landmarksArr[i].y -= pose_center[1]; // 减去y坐标  
        landmarksArr[i].z -= pose_center[2]; // 减去z坐标 
...
//   * Maximum distance from pose center to any pose landmark
  get_pose_size( landmarks: any[]){
      ...
      const hips = [];   
      console.log("mediapipePose.POSE_LANDMARKS.LEFT_HIP",mediapipePose.POSE_LANDMARKS.LEFT_HIP)
      console.log("lefthip right_hip.x",  landmark2d[mediapipePose.POSE_LANDMARKS.LEFT_HIP], right_hip.x)   
     ...
     // --  Torso size as the minimum body size.   torso_size = np.linalg.norm(shoulders - hips)        
        ...
        console.log("landmark2d_s start:", landmark2d_s)
  ...
  get_distance_by_names( landmarks:any[], name_from:number, name_to:number){
    const lmk_from = landmarks[name_from]
    const lmk_to = landmarks[name_to]
    const distance = lmk_from
      distance.x = (lmk_from.x - lmk_to.x) *0.5;
      distance.y = (lmk_from.y - lmk_to.y) *0.5;
      distance.z = (lmk_from.z - lmk_to.z) *0.5;
    return distance;
  }

  get_distance( landmark_from:any, landmark_to:any){
    const distance = landmark_from
      distance.x = (landmark_from.x - landmark_to.x) *0.5;
      distance.y = (landmark_from.y - landmark_to.y) *0.5;
      distance.z = (landmark_from.z - landmark_to.z) *0.5;
    return distance;
  }
